...
function Details() {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:3001/korisnici/byId/${sessionStorage.getItem("user")}`
      )
      .then((response) => {
  ...
  console.log(
    knjige
      ?.filter((knjiga) =>
        iznajmljene.includes(parseInt(knjiga.id_knjige))
          ? knjiga?.naziv_knjige
          : null
      )
      .filter(Boolean)
      .map(({ naziv_knjige, updatedAt }) => {
        return {
          naziv_knjige,
          datum: new Date(
            new Date(updatedAt).getTime() + 1000 * 60 * 60 * 24 * 20
          ),
        };
      })
  );
  ...
  const zakasnina = (updatedAt) => {
    ...
    if (current > deadLine) {
      return <>Zakasnina : 50 kuna</>;
    } else {
      return <>Zakasnina : 0 kuna</>;
    }
  };
  return (
    <div className="App">
      ...
      {knjige
        ...
        .map(({ naziv_knjige, updatedAt }) => {
          return (
            <>
              <p>
                <b>Naziv iznajmljene knjige:</b>
                {naziv_knjige}
              </p>
              <p>
                <b>Knjigu treba vratiti datuma: </b>
                {new Date(
                  new Date(updatedAt).getTime() + 1000 * 60 * 60 * 24 * 20
                ).toString()}
              </p>
